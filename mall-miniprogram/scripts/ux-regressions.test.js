...
const catalogSource = read('pages/catalog/index.ts')
assert.doesNotMatch(catalogMarkup, /category-scroll/)
assert.match(catalogMarkup, /class="category-trigger"[^>]*bindtap="openCategoryPanel"/)
assert.match(catalogMarkup, /wx:if="\{\{categoryOpen\}\}" class="category-mask"[^>]*bindtap="closeCategoryPanel"/)
assert.match(catalogMarkup, /scroll-view class="category-list" scroll-y/)
assert.match(catalogMarkup, /class="category-sheet"[^>]*catchtap="noop"/)
...
assert.match(catalogStyles, /\.category-trigger\s*\{[^}]*display:\s*flex[^}]*height:\s*44px/s)
assert.match(catalogStyles, /\.category-mask\s*\{[^}]*position:\s*fixed[^}]*inset:\s*0/s)
assert.match(catalogStyles, /\.category-list\s*\{[^}]*max-height:\s*52vh/s)
assert.match(catalogStyles, /\.category-grid\s*\{[^}]*display:\s*grid[^}]*grid-template-columns:\s*repeat\(2,minmax\(0,1fr\)\)/s)
assert.match(catalogSource, /categoryOpen:\s*false/)
assert.match(catalogSource, /selectedCategoryName:\s*'全部'/)
assert.match(catalogSource, /openCategoryPanel\(\)/)
assert.match(catalogSource, /closeCategoryPanel\(\)/)
assert.match(catalogSource, /openCategoryPanel\(\)[^]*setData\(\{ hidden: true \}\)/s)
assert.match(catalogSource, /closeCategoryPanel\(\)[^]*setData\(\{ hidden: false \}\)/s)
assert.match(catalogSource, /categoryOpen:\s*false[^]*this\.loadCatalog\(\)/s)

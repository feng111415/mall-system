...
- **V0.3 模块 6**：商城用户端全站视觉统一、可读性与信息层级提升、桌面反馈、移动端弱动态和减少动态效果支持。
...
V0.3 模块 6 已完成用户端视觉升级，继续复用原有 Vue 3、Vant、Pinia、Vue Router 和真实业务接口，未改动后端接口、若依运营端或 V0.4 菜单规划。

- 新增共享视觉覆盖层，统一首页、商品列表、商品详情、购物车、结算、订单列表、订单详情、地址簿、个人中心和登录页的色板、字号层级、表单边界和状态提示。
- 使用珊瑚红、薄荷绿、暖黄色和深墨色多色点缀，提高小字号文本、价格、订单状态和错误提示的对比度；保留真实商品图片和现有业务内容。
- 商品卡、订单卡、地址卡和表单控件增加克制的圆角、边界、悬停上浮、图片缩放及按钮按压反馈；移动端关闭上浮和缩放，固定底部导航保留安全区留白。
- 增加键盘焦点可见样式，并统一支持 `prefers-reduced-motion: reduce`，减少动态效果时不影响业务操作。
- 页脚版本标记更新为 V0.3；`ai-web/`、视觉原型、原业务库 `ry-vue` 和运营端未修改。

验证结果：

- `npm.cmd run build`（`mall-storefront`）：通过，Vite 转换 374 个模块。
...
- Edge 1440px/390px 检查首页、商品列表、商品详情、购物车、结算、订单、个人中心和登录重定向；`scrollWidth` 等于视口宽度，无图片失败、控制台错误或失败请求。

  ...
  <div class="app-container">
    <el-tabs v-model="activeTab" @tab-click="handleTabClick">
      ...
      <el-tab-pane label="告警中心" name="alerts">
        <el-form :model="alertQuery" size="small" :inline="true" label-width="80px"><el-form-item label="告警状态"><el-select v-model="alertQuery.status" clearable placeholder="全部状态"><el-option label="待确认" value="OPEN"/><el-option label="已确认" value="ACKED"/></el-select></el-form-item><el-form-item><el-button type="primary" icon="el-icon-search" size="mini" @click="getAlerts">查询</el-button></el-form-item></el-form>
...
export default {
  ...
  data() { return { activeTab: 'diffs', showSearch: true, loading: false, alertLoading: false, diffList: [], alertList: [], diffTotal: 0, alertTotal: 0, diffQuery: { pageNum: 1, pageSize: 10, diffType: undefined, status: undefined, businessNo: undefined }, alertQuery: { pageNum: 1, pageSize: 10, status: undefined } } },
  created() { this.getDiffs() },
  methods: {
    ...
    getAlerts() { this.alertLoading = true; listAlerts({ status: this.alertQuery.status, limit: this.alertQuery.pageSize, offset: (this.alertQuery.pageNum - 1) * this.alertQuery.pageSize }).then(r => { const d = r.data || r; this.alertList = d.rows || (Array.isArray(d) ? d : []); this.alertTotal = d.total || this.alertList.length }).finally(() => { this.alertLoading = false }) },

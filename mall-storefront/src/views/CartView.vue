...
<template>
  <section class="cart-wrap">
    ...
    <div v-else-if="cart.items.length" class="cart-list">
      ...
      <div class="cart-summary"><div><span>已选商品合计</span><strong>¥{{ formatPrice(cart.totalPrice) }}</strong><p v-if="message || !cart.canCheckout" class="form-message">{{ message || '请处理失效或库存不足的商品后再结算' }}</p></div><button class="primary-button" :disabled="!cart.canCheckout || busy || !checkoutReady" @click="router.push('/checkout')">去结算 <span>→</span></button></div>
    </div>
